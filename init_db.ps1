...
$SqlFile = Join-Path $SqlDir "account_init.sql"
...
try {
    ...
    # Secondary confirmation to prevent accidental execution (will drop and recreate account_db)
    $confirm = Read-Host "This will drop and recreate the account_db database! Type y to confirm, any other key to cancel"
    if ($confirm -ne 'y') {
    ...
    Get-Content -Encoding UTF8 account_init.sql | mysql -h 127.0.0.1 -P 3306 -u root -proot123456
